...
def get_iterative_process_for_minimal_sum_example():
  ...
  def next_fn(server_state, client_data):
    ...
    secure_modular_sum_update = value_impl.Value(
        context_stack_impl.context_stack.current.bind_computation_to_reference(
            building_block_factory.create_federated_secure_modular_sum(
                client_updates[3].comp,
                modulus=value_impl.to_value(8, tf.int32).comp,
...
class GetComputationForDistributeAggregateFormTest(FederatedFormTestCase,
...
class GetDistributeAggregateFormTest(FederatedFormTestCase,
                                     parameterized.TestCase):
  ...
  def test_temperature_example_round_trip(self):
    ...
    comp = form_utils.get_computation_for_distribute_aggregate_form(example.daf)
    new_initialize = (
        form_utils.get_state_initialization_computation(example.initialize))
...
class GetComputationForMapReduceFormTest(FederatedFormTestCase,
                                         parameterized.TestCase):
...
class CheckMapReduceFormCompatibleWithComputationTest(FederatedFormTestCase,
                                                      parameterized.TestCase):
