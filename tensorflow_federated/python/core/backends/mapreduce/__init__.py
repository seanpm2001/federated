...
def round_comp(server_state, client_data):
  ...
  # a temporary server state that may be used by later stages.
  context_at_server, temp_server_state = tff.federated_map(server_prepare,
  server_state)

  # Broadcast context_at_server to the clients according to the intrinsics
  # within server_to_client_broadcast to produce context_at_clients.

  # The clients all independently do local work and produce updates.
  work_at_clients = tff.federated_map(client_work, [client_data,
  context_at_clients])

  # Aggregate work_at_clients at the server according to the intrinsics within
  # client_to_server_aggregation to produce intermediate_result_at_server.

  # Finally, the server produces a new state as well as server-side output to
  # emit from this round.
  new_server_state, server_output = tff.federated_map(server_result,
  [server_data, temp_server_state, intermediate_result_at_server])
...
* `server_prepare` represents the preparatory steps taken by the server to
  ...
  server state that may be needed by the `client_to_server_aggregation` and
  `server_result` components. Its type signature is `(S -> <B_I, T>)`.

* `server_to_client_broadcast` represents the broadcast of data from the server
  to the clients. Its type signature is `(B_I -> B_O)`.
...
* `client_work` represents the totality of client-side processing. It takes a
  ...
  returns the client update to be aggregated (across all the clients). Its type
  signature is `(<D, B_O> -> A_I)`.

* `client_to_server_aggregation` represents the aggregation of data from the
  clients to the server. It may incorporate the temporary server state that
  was generated by the `server_prepare` component to set dynamic aggregation
  parameters. Its type signature is `(<T, A_I> -> A_O)`.

* `server_result` represents the post-processing steps taken by the server.
  It may depend on the original server state, the temporary server state that
  was generated by the `server_prepare` component, and the data that was
  aggregated from the clients. It will generate a new server state to feed into
  the next round and an additional server-side output to be reported externally
  as one of the results of this round. In federated learning scenarios, the
  server-side outputs might include things like loss and accuracy metrics, and
  the server state to be carried over may include the model weights. Its type
  signature is `(<S, T, A_O> -> <S, X>)`.
...
* `T` is the type of the temporary server state.
"""
