...
def _is_assignable_from_or_both_none(first, second):
  ...
  return bool(first.is_assignable_from(second))
...
class DistributeAggregateForm(typed_object.TypedObject):
  ...
  This standardized representation can be used to describe a range of
  computations that constitute one round of processing in a standard federated
  learning system. An example of such a federated learning system is described
  ...
  def __init__(
    ...
    _check_returns_tuple('server_prepare', server_prepare, 2)
    ...
    # to aggregate.
    _check_accepts_tuple('client_work', client_work, 2)

    # The client_to_server_aggregation function should take 2 inputs (temporary
    # server state and client results) and produce an output of arbitrary
    # length that represents the aggregated data.
    _check_accepts_tuple('client_to_server_aggregation',
                         client_to_server_aggregation, 2)

    # The server_result function should take 3 inputs (server state, temporary
    # server state, and aggregate client data) and produce 2 outputs (new
    # server state and server output)
    _check_accepts_tuple('server_result', server_result, 3)
    _check_returns_tuple('server_result', server_result, 2)
    ...
    # The temporary server state data types in the 'server_prepare' result,
