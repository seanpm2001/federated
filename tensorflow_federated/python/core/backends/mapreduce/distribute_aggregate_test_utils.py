...
def generate_unnamed_type_signature(client_work, server_result):
...
def get_temperature_sensor_example():
  ...
  def server_prepare(state):
    ...
    intermediate_state = []
    return [[intrinsics.federated_map(server_prepare_tf,
                                      state)]], intermediate_state

  # The broadcast input and output types are both singleton tuples containing
  # a single float `max_temperature`, which is the threshold received from the
  # server.
  broadcast_type = collections.OrderedDict(max_temperature=tf.float32)

  # The intermediate state type is an empty struct.
  ...
  def client_work(data, context_at_client):
    ...
    return [
        value_impl.Value(
            ...
            .bind_computation_to_reference(
                building_block_factory.create_federated_unzip(
                    (intrinsics.federated_map(
                        client_work_tf, (data, context_at_client))).comp)))
    ]

  # The client update is a tuple.
  federated_client_update_type = collections.OrderedDict(
      is_over=computation_types.at_clients(tf.float32),
      weight=computation_types.at_clients(tf.float32))

  @federated_computation.federated_computation(intermediate_state_type,
  ...
  def client_to_server_aggregation(intermediate_server_state, client_updates):
    ...
    return [
        intrinsics.federated_mean(client_updates[0][0], client_updates[0][1])
    ]

  # The aggregation result type is a struct containing a single float.
  aggregation_result_type = tf.float32

  @federated_computation.federated_computation(
      computation_types.at_server(server_state_type), intermediate_state_type,
      [computation_types.at_server(aggregation_result_type)])
  def server_result(state, intermediate_server_state, aggregation_result):
    del intermediate_server_state  # Unused
    ...
    return intrinsics.federated_map(
        server_result_tf, state), collections.OrderedDict(
            ratio_over_threshold=aggregation_result[0])

  type_signature = generate_unnamed_type_signature(client_work, server_result)
  return DistributeAggregateFormExample(
      daf=forms.DistributeAggregateForm(type_signature, server_prepare,
...
def get_mnist_training_example():
    ...
    client_update = value_impl.Value(
        context_stack_impl.context_stack.current.bind_computation_to_reference(
            building_block_factory.create_federated_unzip(
                intrinsics.federated_map(
                    client_work_tf,
                    intrinsics.federated_zip([data,
                                              context_at_client[0]])).comp)))
    return [[
        client_update.model,
        intrinsics.federated_map(cast_to_float, client_update.num_examples)
            ...
            [
                client_update.loss,
                intrinsics.federated_map(cast_to_float,
                                         client_update.num_examples)
            ]]
  ...
  federated_aggregation_input_tff_type = [
      ...
      [computation_types.at_clients(tf.int32)],  # input for federated_sum
      [
          computation_types.at_clients(tf.float32),
          computation_types.at_clients(tf.float32)
      ]  # input for federated_mean
  ...
  def client_to_server_aggregation(intermediate_server_state, client_updates):
    del intermediate_server_state  # Unused
    scaled_model = intrinsics.federated_mean(client_updates[0][0],
                                             client_updates[0][1])
    num_examples = intrinsics.federated_sum(client_updates[1][0])
    scaled_loss = intrinsics.federated_mean(client_updates[2][0],
                                            client_updates[2][1])
  ...
  @federated_computation.federated_computation(
      computation_types.at_server(server_state_tff_type),
      intermediate_state_type, federated_aggregation_result_type)
  def server_result(state, intermediate_server_state, aggregation_result):
    del intermediate_server_state  # Unused
    ...
    num_rounds = intrinsics.federated_map(server_result_tf, state)

    return intrinsics.federated_zip(
        server_state_nt(model=aggregation_result.model,
                        num_rounds=num_rounds)), intrinsics.federated_zip(
                            metrics_nt(
